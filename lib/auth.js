...
import { NO_ACCOUNT_ERROR } from "./authErrors";
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Distinguish "no account at all" so the client can offer to create one with
        // the entered email/password. NextAuth surfaces a *thrown* error's message to
        // signIn() (returning null would collapse to the generic "CredentialsSignin").
        if (!user) throw new Error(NO_ACCOUNT_ERROR);
        // Account exists but has no password (e.g. signed up via Google) — keep generic.
