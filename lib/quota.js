...
// Callers must `await connectDB()` first (bufferCommands is off in this repo).
// ok:false is also returned when the user doc doesn't exist (planned callers
// load the user first, so they never hit that ambiguity).
export async function lifetimeQuota(userId, field, limit) {
  const updated = await User.findOneAndUpdate(
    {
      _id: userId,
      $or: [{ [field]: { $lt: limit } }, { [field]: { $exists: false } }],
    },
    { $inc: { [field]: 1 } },
    { new: true }
  ).select(field);
...
// Note: `count` keeps incrementing past the limit, so never derive a
// "remaining" value from it.
